CREATE PROCEDURE [dbo].[AMC_spValidateStudentExamAnswerKey]
 @StudentID int,
 @Class char(2),
...
BEGIN
		...
		Select   @StudentName =SM.colStudentFName + ' '+  SM.colStudentLName
				...
				,@Group=Case	when @mClass='JB' then 'Junior Beginner - ' + Section
								...
								when @mClass='GD' Then 'Game Development'
								when @mClass='AD' Then 'App Development'
						END
		...
		Where StudentID=@StudentID
		 ...
		 ------ Checkling the Answer-----------------------------------------
				...
				IF @ChapterID=5 
				 BEGIN 
						IF (@ExamType='Final Exam' OR  @ExamType='Mock Test')
							BEGIN 
									Declare @TotalCorrect int
									Declare @TotalIncorrect int
									Declare @TotalRawScore int
									Declare @TotalNotAnswered int
									Set @TotalCorrect=0
									Set @TotalIncorrect=0
									Set @TotalRawScore=0
									Set @TotalNotAnswered=0
									Set @FinalExamTotalScore=800
									Select @TotalNotAnswered=Count(*) from [AMC_ExamMasterAnswerKey] with (NOLOCK)
									where StudentID=@StudentID and Class=@Class and AnswerKey='' and Semester=@mCurrentSemester
									and ExamType=@ExamType and Session=@Session

									Set @TotalCorrect=@FinalExamReceivedScore
									Set @TotalIncorrect=50-(@TotalCorrect+@TotalNotAnswered)
									Set @TotalRawScore= Round(@TotalCorrect-(.25* @TotalIncorrect),0)
									
									Select @FinalExamReceivedScore =ReportedScore from AMC_ExamMaster_RowScoreLookup with (NOLOCK)
									where ChapterID=@ChapterID and ExamType=@ExamType 
									and  Rtrim(Session)=Rtrim(@Session) 
									and Semester=@mCurrentSemester
									and RawScore=@TotalRawScore

									Set @FinalExamComments= case when @FinalExamReceivedScore>750 then 'Excellent Job.'
											 when @FinalExamReceivedScore between 700 and 750 then 'Good Job.'	
											else 'Good Try. Practice more.'	
										End 
									Set @FinalExamComments='Your answered '+ convert(char(2),@TotalCorrect) + ' correctly . '+ @FinalExamComments
			  ...
			  ---Update the Priority Registrationh 
		  IF Rtrim(@ExamType)='Final Exam'
		  BEGIN 
